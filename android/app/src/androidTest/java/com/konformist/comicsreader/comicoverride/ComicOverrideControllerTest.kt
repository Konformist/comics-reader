...
class ComicOverrideControllerTest {
  private lateinit var db: AppDatabase
  private lateinit var controller: ComicOverrideController
  ...
  fun setUp() {
    ...
    controller = ComicOverrideController(db.comicOverrideDao())
  }
  ...
  fun testUpdateNullable() {
    ...
    val parserConfigUpdate = ComicOverrideUpdate(id = rowId)

    Thread.sleep(1000)
    controller.update(parserConfigUpdate)
    val rowUpdated = controller.read(rowId)
  ...
  fun testDelete() {
    ...
    val rowId = controller.create(create)
    controller.delete(ComicOverrideDelete(id = rowId))
    val row = controller.read(rowId)
